...
// console.log(decodeURI(decodeURI(GetQueryString("keyWord"))));
...
$(function() {
  ...
  $.ajax({
    type: "get",
    url: common_api + "/user/search.action?keyWord=" + decodeURI(decodeURI(GetQueryString("keyWord"))),
    dataType: "json",
    success: function(data) {
      console.log(data);
      if (data.code === 0) {
        $(".more").html('亲,暂无数据了呢~请 <a href="javascript: history.go(-1)" style="color:red;font-size:20px;"> 返回 </a> 重新查询吧');
      } else {
        var lis = "";
