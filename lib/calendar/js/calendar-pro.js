(function(){
	...
	calendarDate.timeDate = calendarDate.today.getDate();//当前日
	...
	function getIndexDay(){
		...
		for( var k = 0;k<calendarDate.days;k++ ){
			...
			for( var d in calendarDate.opt.data ){
				...
				var dataDate = calendarDate.opt.data[d].timeDate; 
				flag = checkDate(nowDate,dataDate);
				if( flag ){
					riliHtml += '<div class="ht-rili-td ht-rili-onclick" data-date="' + dataDateStr + '"><span class="ht-rili-day">' + (k + 1) + '</span><span class="ht-rili-money" data-money="' + calendarDate.opt.data[d].price + '">&yen;' + calendarDate.opt.data[d].price + "</span></div>";
					break;
				}
			}
			if( !flag ){
	...
	$.fn.extend({
	    calendar:function(opt){
	        if( (opt.beginDate != undefined && opt.endDate != undefined )||( opt.data.length > 0 ) ){
	        	var beginDate = opt.data[0].timeDate;
	        	var endDate = opt.data[opt.data.length - 1].timeDate;
				...
				calendarDate.timeDate = parseInt(beginDate.split("-")[2]);//设置起始日期为当前日期
